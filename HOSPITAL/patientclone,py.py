...
def insert():
    ...
    c.execute("SELECT PatientId FROM Patient9;")
    data = c.fetchall()
...
def fetch_patient_info(patient_id):
    try:
        ...
        cursor.execute("SELECT PatientName,Address,DateOfBirth,Diagnosis,FatherName,MotherName,BloodGroup,PhoneNumber,Prescription FROM Patient9 WHERE PatientId = %s;", (patient_id,))
...
l27.place(x=900,y=40)

l26=customtkinter.CTkLabel(master=l5,text=" ",text_color="black",font=("times new roman",20,"bold"),fg_color="white",bg_color="#cfe2f3",corner_radius=6,width=300,height=340)
l26.place(x=900,y=100)
