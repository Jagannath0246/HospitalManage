...
def insert():
    ...
    c.execute("SELECT PatientId FROM Patient10;")
    data = c.fetchall()
...
def fetch_patient_info(patient_id):
    try:
        ...
        cursor.execute("SELECT PatientName,Address,DateOfBirth,Diagnosis,FatherName,MotherName,BloodGroup,PhoneNumber,Prescription,Photo FROM Patient10 WHERE PatientId = %s;", (patient_id,))
